...
def SequentialEMforMixGaussian(InputData, K):
    ...
    for n_iter in range(1, N):
        ...
        for k_iter in range(K):
            rZnk_new[n_iter, k_iter] = pi_Cof[0, k_iter] * NDimensionGaussian(X[:, n_iter], UK[:, k_iter],
                                                                              List_cov[k_iter])
        ...
        for k_iter in range(K):
            ...
            UK_new[:, k_iter] = UK[:, k_iter] + ((rZnk_new[n_iter, k_iter] - rZnk[n_iter, k_iter]) / float(
                Nk_new[0, k_iter])) * (X[:, n_iter] - UK[:, k_iter])
            print('UK_new', UK_new, '\n')
            Temp = X[:, n_iter] - UK_new[:, k_iter]
            List_cov_new[k_iter] = List_cov[k_iter] + ((rZnk_new[n_iter, k_iter] - rZnk[n_iter, k_iter]) / float(
                Nk_new[0, k_iter])) * (Temp * Temp.transpose() - List_cov[k_iter])
    ...
    return pi_Cof, UK_new, List_cov, rZnk_new
...
def BatchEMforMixGaussian(InputData, K, MaxIter):
    ...
    for d_iter in range(D):
        for k_iter in range(K):
            UK[d_iter, k_iter] = X_mean[d_iter, 0] + (-1) ** k_iter + (-1) ** d_iter
    print(UK)
    UK[:,0]=InputData[:,5].reshape((2,1))
    UK[:,1]=InputData[:,21].reshape((2,1))
    UK[:,2]=InputData[:,26].reshape((2,1))
    # 初始化k个协方差矩阵的列表
    List_cov = []

    for k_iter in range(K):
        List_cov.append(numpy.mat(numpy.eye(X[:, 0].size))*0.1)
    print(List_cov)
    ...
    while True:
        ...
        for n_iter in range(N):
            for k_iter in range(K):
                rZnk[n_iter, k_iter] = pi_Cof[0, k_iter] * NDimensionGaussian(X[:, n_iter], UK[:, k_iter],
                                                                              List_cov[k_iter])
